...
def get_video_duration(file_path):
    clip = VideoFileClip(file_path)  # Load the video file
    return clip.duration  # Return the duration of the video
...
def calculate_total_time(folder_path):
    ...
    # Append a row for the total duration to the video_data list
    video_data.append(["", "", ""])  # Add empty row for spacing
    video_data.append(["Total", "", f"{total_hours} hours, {total_minutes} minutes, {total_seconds} seconds"])
...
# Main function to prompt user for input and call the calculation function
def main():
    ...
    calculate_total_time(path)  # Call the function to calculate total video duration
